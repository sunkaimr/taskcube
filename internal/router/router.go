...
func Init(ctx *common.Context) *gin.Engine {
	...
	// 任务相关路由
	tmpl := public.Group("/task-template")
	{
		// 创建任务模版
		tmpl.POST("/", new(ctl.TaskController).UpdateTask)
		// 删除任务模版
		tmpl.DELETE("/", new(ctl.TaskController).DeleteTask)
		// 修改任务模版
		tmpl.PUT("/", new(ctl.TaskController).UpdateTask)
		// 查询任务模版
		tmpl.GET("/", new(ctl.TaskController).QueryTask)
		// 提交任务
		tmpl.POST("/:task-template/submit", new(ctl.TaskController).QueryTask)
	...
	task := public.Group("/task")
	{
		...
		// 暂停任务
		task.POST("/", new(ctl.TaskController).QueryTask)
		// 终止任务
		task.POST("/", new(ctl.TaskController).QueryTask)
		// 恢复任务
		task.POST("/", new(ctl.TaskController).QueryTask)
